...
description: Converts a reviewed QA story and its grill output into a testable QA specification with functional, negative, boundary, integration, and validation coverage, plus required test data and environment dependencies.
version: 0.2.0
---
...
- The Requirement Analysis output for the same story, if available.
- The Risk Analysis output for the same story, if available.
- The Test Architecture output for the same story, if available.
...
Read the story intake, acceptance criteria, grill output, analysis output, risk output, architecture output, discovery report, and project adapter. Understand the requirement, the acceptance criteria, the ambiguities and gaps identified by the grill, the risks identified by risk analysis, and the architecture decisions.
...
### 3. Identify acceptance criteria dynamically

Parse the acceptance criteria from the story intake. Each acceptance criterion should be identified by its heading (e.g., "### AC1 — ...") and its body. The number of acceptance criteria is dynamic — the specification must handle any number.

### 4. Define functional coverage

For each acceptance criterion, define the functional coverage:

- The primary behavior that must work.
- The acceptance criterion that maps to each behavior.
...
### 8. Define validation expectations
...
### 10. Identify required environment dependencies
...
### 12. Identify items blocked pending clarification

For each item that is blocked by an unresolved ambiguity, missing requirement, or unknown from prior stages, record the item and what is blocking it.
...
2. Functional coverage (one subsection per acceptance criterion, dynamically generated)
3. Negative coverage
4. Boundary coverage
...
- The number of acceptance criteria handled is dynamic — the specification does not assume a fixed count.
